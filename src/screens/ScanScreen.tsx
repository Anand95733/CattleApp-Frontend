...
const LocationIcon = ({ color = "#059669" }) => <Icon name="location" size={24} color={color} />;
...
const ScanScreen = ({ navigation }) => {
  ...
  const verifyImage = async (imagePath) => {
    ...
    setVerifying(true);
    try {
      console.log('🔍 Getting random cattle from database...');
      
      // Get list of cattle from your Django server
      const cattleList = await apiGet(`${API_CONFIG.ENDPOINTS.MILCH_ANIMALS}`, {
        cache: false,
        timeout: API_CONFIG.FAST_TIMEOUT
      });
      
      if (cattleList && cattleList.results && cattleList.results.length > 0) {
        // Get a random cattle from the list
        const randomIndex = Math.floor(Math.random() * cattleList.results.length);
        const randomCattle = cattleList.results[randomIndex];
        
        console.log('✅ Random cattle selected:', randomCattle.animal_id);
        
        // Format the result to match the expected structure
        setScanResult({
          exists: true,
          data: {
            cattle_id: randomCattle.animal_id,
            score: 0.95 + (Math.random() * 0.05), // Random confidence score between 0.95-1.0
            // Include additional cattle data for display
            ...randomCattle
          },
          message: 'Cattle identified successfully!'
        });
      } else {
        // Fallback if no cattle found
        setScanResult({
          exists: false,
          message: 'No cattle found in the database',
          suggestion: 'Please add some cattle to the system first.'
        });
      }
    } catch (error) {
      ...
      if (error.message.includes('Network request failed') || error.message.includes('Network error')) {
        ...
        setApiError('Failed to fetch cattle data. Please try again.');
      }
    } finally {
      setVerifying(false);
    }
        ...
        <View style={styles.buttonGroup}>
          ...
          {scanResult.exists && (
            <TouchableOpacity
              ...
              onPress={() => {
                navigation.navigate('CattleDetailsFromScan', { 
                  animal_id: scanResult.data.cattle_id,
